...
public class GuardController : MonoBehaviour
{
    ...
    void AnimasiIdle()
    {
        if (this.GetComponent<UnityEngine.AI.NavMeshAgent>().remainingDistance < 0.1f)
        {
    ...
    void Chase(Transform player)
    {
        ...
        Vector3 direction = player.position - this.transform.
        position;
        this.transform.rotation = Quaternion.
        Slerp(this.transform.rotation, Quaternion.
        LookRotation(direction), Time.deltaTime * this.
        chasingRotSpeed);
    ...
    void Investigate()
    {
        ...
        {
            this.GetComponent<UnityEngine.AI.NavMeshAgent>().
            SetDestination(lastPlaceSeen);
    ...
    void Patrol()
    {
        ...
        if (patrolTimePassed > patrolWait)
        {
            ...
            patrollingPoint += new Vector3(Random.Range
            (-patrolDistance, patrolDistance), 0, Random.
            Range(-patrolDistance, patrolDistance));

            // Make the generated point a goal for the agent
            this.GetComponent<UnityEngine.AI.NavMeshAgent>().
            SetDestination(patrollingPoint);
        }
    }
